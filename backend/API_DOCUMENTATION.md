...
| GET    | `/users`     | List users (optional `?role=developer&accountStatus=suspended`) | Admin               |
| GET    | `/users/:id` | Get user                                                        | Self or Admin       |
| PUT    | `/users/:id` | Update user (e.g. name)                                         | Self or Super Admin |

`/users` and `/users/:id` now include latest account status metadata:
...
All endpoints below require **Super Admin** role.

| Method | Endpoint                                             | Purpose                                   |
| ------ | ---------------------------------------------------- | ----------------------------------------- |
| GET    | `/super-admin/users/account-status?status=suspended` | List users by latest account status       |
| POST   | `/super-admin/users/:userId/pause`                   | Pause an account (reversible)             |
| POST   | `/super-admin/users/:userId/resume`                  | Resume paused/blocked account             |
| DELETE | `/super-admin/users/:userId/permanent`               | Permanently delete account (irreversible) |

**Pause body:** `{ "reason": "...", "suspensionExpiresAt?": "ISO datetime" }`  
**Resume body:** `{ "reason?": "..." }`  
**Permanent delete body:** `{ "reason": "...", "password": "<super-admin-password>" }`

**Security behavior:**

- Paused/locked/pending-deletion/banned users are blocked at auth middleware and login (`403`).
- Permanent delete writes audit markers and blocks future re-registration with the same email.
- Self-pause and self-permanent-delete are rejected.
